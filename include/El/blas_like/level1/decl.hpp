...
*/
#ifndef EL_BLAS_LIKE_LEVEL_1_DECL_HPP
#define EL_BLAS_LIKE_LEVEL_1_DECL_HPP
...
(Ring alpha, const ElementalMatrix<Ring>& A, DistMatrix<Ring,Dist::STAR,Dist::STAR>& B);
...
void ColumnTwoNorms
(const DistMatrix<Field,U,V,W>& X, DistMatrix<Base<Field>,V,Dist::STAR,W>& norms);
...
void ColumnMaxNorms
(const DistMatrix<Ring,U,V,W>& X, DistMatrix<Base<Ring>,V,Dist::STAR,W>& norms);
...
void RowTwoNorms
(const DistMatrix<Field,U,V>& X, DistMatrix<Base<Field>,U,Dist::STAR>& norms);
...
template<typename Ring,Dist U,Dist V>
void RowMaxNorms
(const DistMatrix<Ring,U,V>& X, DistMatrix<Base<Ring>,U,Dist::STAR>& norms);
...
template<typename T>
void CopyFromRoot
(const Matrix<T>& A, DistMatrix<T,Dist::CIRC,Dist::CIRC>& B,
  bool includingViewers=false);
template<typename T>
void CopyFromNonRoot(DistMatrix<T,Dist::CIRC,Dist::CIRC>& B,
  bool includingViewers=false);
...
(const Matrix<T>& A, DistMatrix<T,Dist::CIRC,Dist::CIRC,DistWrap::BLOCK>& B,
  bool includingViewers=false);
template<typename T>
void CopyFromNonRoot(DistMatrix<T,Dist::CIRC,Dist::CIRC,DistWrap::BLOCK>& B,
  bool includingViewers=false);
...
template<typename TDiag,typename T,Dist U,Dist V,DistWrap wrapType=DistWrap::ELEMENT>
void DiagonalScale
(LeftOrRight side, Orientation orientation,
...
(LeftOrRight side, UpperOrLower uplo, Orientation orientation,
  const AbstractDistMatrix<TDiag>& d,
        DistMatrix<T,U,V,DistWrap::BLOCK>& A, Int offset=0);
...
(LeftOrRight side, Orientation orientation,
  const AbstractDistMatrix<FieldDiag>& d,
        DistMatrix<Field,U,V,DistWrap::BLOCK>& A,
  bool checkIfSingular=true);
...
GetDiagonal(const DistMatrix<T,U,V,DistWrap::BLOCK>& A, Int offset=0);
...
GetRealPartOfDiagonal(const DistMatrix<T,U,V,DistWrap::BLOCK>& A, Int offset=0);
...
template<typename T,Dist U,Dist V>
DistMatrix<Base<T>,Dist::MC,Dist::STAR>
GetImagPartOfDiagonal(const DistMatrix<T,U,V,DistWrap::BLOCK>& A, Int offset=0);

// GetMappedDiagonal
// =================
template<typename T,typename S>
void GetMappedDiagonal
(const Matrix<T>& A, Matrix<S>& d, std::function<S(const T&)> func, Int offset=0);
template<typename T,typename S,Dist U,Dist V>
void GetMappedDiagonal
(const DistMatrix<T,U,V>& A, AbstractDistMatrix<S>& d,
  std::function<S(const T&)> func, Int offset=0);
template<typename T,typename S,Dist U,Dist V>
void GetMappedDiagonal
(const DistMatrix<T,U,V,DistWrap::BLOCK>& A, AbstractDistMatrix<S>& d,
  std::function<S(const T&)> func, Int offset=0);
...
// =================
template<typename T>
void IndexDependentMap(Matrix<T>& A, std::function<T(Int,Int,const T&)> func);
template<typename T>
void IndexDependentMap
(AbstractDistMatrix<T>& A, std::function<T(Int,Int,const T&)> func);

template<typename S,typename T>
void IndexDependentMap
(const Matrix<S>& A,
        Matrix<T>& B,
        std::function<T(Int,Int,const S&)> func);
...
(LeftOrRight side, UpperOrLower uplo,
  const AbstractDistMatrix<FieldMain>& d,
  const AbstractDistMatrix<Field>& dSub,
        DistMatrix<Field,U,V,DistWrap::BLOCK>& X,
        bool conjugated=false);

template<typename Field,typename FieldMain,Dist U,Dist V>
void LeftQuasiDiagonalScale
(UpperOrLower uplo,
  const DistMatrix<FieldMain,U,Dist::STAR>& d,
  const DistMatrix<FieldMain,U,Dist::STAR>& dPrev,
  const DistMatrix<FieldMain,U,Dist::STAR>& dNext,
  const DistMatrix<Field,    U,Dist::STAR>& dSub,
  const DistMatrix<Field,    U,Dist::STAR>& dSubPrev,
  const DistMatrix<Field,    U,Dist::STAR>& dSubNext,
        DistMatrix<Field,U,V>& X,
...
(UpperOrLower uplo,
  const DistMatrix<FieldMain,V,Dist::STAR>& d,
  const DistMatrix<FieldMain,V,Dist::STAR>& dPrev,
  const DistMatrix<FieldMain,V,Dist::STAR>& dNext,
  const DistMatrix<Field,    V,Dist::STAR>& dSub,
  const DistMatrix<Field,    V,Dist::STAR>& dSubPrev,
  const DistMatrix<Field,    V,Dist::STAR>& dSubNext,
        DistMatrix<Field,U,V>& X,
  const DistMatrix<Field,U,V>& XPrev,
  const DistMatrix<Field,U,V>& XNext,
  bool conjugated=false);
...
(Matrix<T>& A, const Matrix<S>& d,
  std::function<void(T&,const S&)> func, Int offset=0);
template<typename T,typename S,Dist U,Dist V>
void UpdateMappedDiagonal
...
template<typename T,typename S,Dist U,Dist V>
void UpdateMappedDiagonal
(DistMatrix<T,U,V,DistWrap::BLOCK>& A, const AbstractDistMatrix<S>& d,
  std::function<void(T&,const S&)> func, Int offset=0);
...
#endif // ifndef EL_BLAS_LIKE_LEVEL_1_DECL_HPP
