  ...
  bool conjugate=false )
{
    ...
    DistMatrix<T,Dist::MC,  Dist::STAR> A1_MC_STAR(g), B1_MC_STAR(g);
    DistMatrix<T,Dist::VR,  Dist::STAR> A1_VR_STAR(g), B1_VR_STAR(g);
    DistMatrix<T,Dist::STAR,Dist::MR  > A1Trans_STAR_MR(g), B1Trans_STAR_MR(g);
    ...
    for( Int k=0; k<r; k+=bsize )
    {
        ...
        LocalTrr2k
        ( UpperOrLower::UPPER, Orientation::NORMAL, Orientation::NORMAL, Orientation::NORMAL, Orientation::NORMAL,
  ...
  Int blockSize=2000 )
{
    ...
    for( Int kOuter=0; kOuter<n; kOuter+=blockSize )
    {
        ...
        Syr2k
        ( UpperOrLower::UPPER, Orientation::NORMAL, alpha, A1.Matrix(), B1.Matrix(), Z.Matrix(),
          conjugate );
        ...
        for( Int kInner=0; kInner<kOuter; kInner+=blockSize )
        {
            ...
            LocalGemm( Orientation::NORMAL, orient, alpha, A1, B2, Z );
            LocalGemm( Orientation::NORMAL, orient, Conj(alpha), B1, A2, Z );
  ...
  bool conjugate=false )
{
    ...
    if( r > weightAwayFromDot*n )
        UN_Dot( alpha, A, B, C, conjugate, blockSizeDot );
    else
        UN_C( alpha, A, B, C, conjugate );
}
