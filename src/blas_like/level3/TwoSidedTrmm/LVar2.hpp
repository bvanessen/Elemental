...
void LVar2( UnitOrNonUnit diag, Matrix<F>& A, const Matrix<F>& L )
{
    ...
    for( Int k=0; k<n; k+=bsize )
    {
        ...
        // A10 := A10 + L21' A20
        Gemm( Orientation::ADJOINT, Orientation::NORMAL, F(1), L21, A20, F(1), A10 );
  ...
  const AbstractDistMatrix<F>& LPre )
{
    ...
    DistMatrix<F,Dist::STAR,Dist::STAR> A11_STAR_STAR(g), L11_STAR_STAR(g),
                            X11_STAR_STAR(g);
    DistMatrix<F,Dist::STAR,Dist::MR  > L21Adj_STAR_MR(g), X10_STAR_MR(g);
    DistMatrix<F,Dist::STAR,Dist::VR  > A10_STAR_VR(g);
    DistMatrix<F,Dist::MC,  Dist::STAR> L21_MC_STAR(g), Z21_MC_STAR(g);
    DistMatrix<F,Dist::MR,  Dist::STAR> Z21_MR_STAR(g);
    ...
    for( Int k=0; k<n; k+=bsize )
    {
        ...
        LocalGemm( Orientation::ADJOINT, Orientation::NORMAL, F(1), L21_MC_STAR, A20, X10_STAR_MR );
