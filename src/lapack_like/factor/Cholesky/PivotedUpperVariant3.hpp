...
void PivotedUpperUnblocked( Matrix<F>& A, Permutation& P )
{
    ...
    for( Int k=0; k<n; ++k )
    {
        ...
        HermitianSwap( UpperOrLower::UPPER, A, k, from );
        P.Swap( k, from );
        ...
        Conjugate( a12 );
        Her( UpperOrLower::UPPER, -F(1), a12, A22 );
        Conjugate( a12 );
    }
}
  ...
  Int off )
{
    ...
    for( Int k=0; k<bsize; ++k )
    {
        ...
        // Apply the pivot
        HermitianSwap( UpperOrLower::UPPER, AFull, k+off, from+off );
        ...
        Gemv( Orientation::NORMAL, F(-1), YB0, x10, F(1), a1R );
        a1R.MakeReal(0,0);
  ...
  Int off )
{
    ...
    for( Int k=0; k<bsize; ++k )
    {
        ...
        if( a1R.ColAlign() == a1R.ColRank() )
            LocalGemv( Orientation::NORMAL, F(-1), YB0, x10, F(1), a1R );
        a1R.MakeReal(0,0);
...
void PivotedUpperVariant3Blocked( Matrix<F>& A, Permutation& P )
{
    ...
    for( Int k=0; k<n; k+=bsize )
    {
        ...
        Trrk( UpperOrLower::UPPER, Orientation::NORMAL, Orientation::TRANSPOSE, F(-1), X21, Y21, F(1), A22 );
    }
}
  ...
  DistPermutation& P )
{
    ...
    for( Int k=0; k<n; k+=bsize )
    {
        ...
        auto X21 = XB1( ind2Pan, ind1Pan );
        auto Y21 = YB1( ind2Pan, ind1Pan );
        LocalTrrk( UpperOrLower::UPPER, Orientation::TRANSPOSE, F(-1), X21, Y21, F(1), A22 );
    }
}
